...
uses
  ...
  TaskDialog, AdvOfficeButtons, RTFLabel
...
type
  Tfrm_PrinterPP00 = class(TBaseForm)
    ...
    lbl_Alert: TLabel;
    chk_IndParcial: TAdvOfficeCheckBox;
    RTFLabel1: TRTFLabel;
...
begin
    ...
    //
    // chk
    // Indicador de impressão parcial
    {if chk_IndParcial.Checked then
    begin
        pp0 :=pp00List.IndexOfParcial ;
        if pp0 <> nil then
            pp1 :=pp0.IndexOfParcial;
        //
        // se não for a porta atual
        if(pp1 <> nil)and(pp1.pp1_codseq <> pp01.pp1_codseq) then
        begin
            msg :=Format('Já existe o ponto (%s\%s) definido como impressão parcial!'#13#10,[pp0.pp0_descri,pp1.pp1_terminalport]);
            msg :=msg +'Deseja sobrepor?';
            if not CMsgDlg.Warning(msg, True) then
            begin
                cbx_PPro.SetFocus ;
                Exit ;
            end;
    ...
    pp01.pp1_indparcial :=chk_IndParcial.Checked ;
    ...
    try
        ...
        begin
            pp0 :=pp00List.Items[cbx_PPGrupo.ItemIndex] ;
            pp00.pp0_codppr :=pp0.pp0_codseq ;
        end
        else
            pp00.pp0_codppr :=-1 ;
...
begin
    ...
    p1.DoPreparePrint(doc, p00, True);
    if doc <> '' then
    begin
...
begin
    ...
    begin
        ...
        //
        // Busca a porta pelo terminal(name)
        //
        ...
        begin
            ...
            // indicador de impressão parcial
            //
            chk_IndParcial.Checked :=pp01.pp1_indparcial ;
        end
        else begin
